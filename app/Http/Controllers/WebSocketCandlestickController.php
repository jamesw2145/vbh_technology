...
use App\Api\CandlestickStreaming;

class WebSocketCandlestickController extends Controller implements MessageComponentInterface {
    private $connections = [];

    private $loop;

    private $candlestickStreaming = new CandlestickStreaming();
    ...
    function initializeLoop() {
        ...
        // Start a looping timer every 30 seconds.
        $this->loop = React\EventLoop\Factory::create();
        $this->loop->addPeriodicTimer(30, function ($timer) use ($process) {
    ...
    function candlestickData () {
        $arr = $candlestickStreaming->getNewCandlestickData();
        foreach ($arr as $candlestick) {
            $json = json_encode($candlestick);
